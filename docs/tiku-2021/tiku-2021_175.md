...
## 5
        ...
        运维配置管理，通常会使用 puppet，那么 master 端和 client 端基于哪种认证方式通信的
...
[一条咸鱼晨](https://www.nowcoder.com/profile/107959225)

                                                                    三次握手的第二次握手，服务器返回的确认号为 seq+1，即为服务器确认从客户端接收的数据字字节数。IP 数据包的最大分配单元固定为 1500B，其中首部部分为 20B，数据部分为 1480.多出 1480 部分将被分片。三次握手结束之后，开始两个程序之间数据交换

发表于 2021-04-09 22:39:55
...
```
一个带宽为 10Mb/s 的集线器连接 8 台计算机，当 8 台计算机同时工作时，每台计算机的平均带宽为 1.25Mb/s
```cpp

```
一般来讲，由集线器连接的网络的拓扑结构属于星型结构
```cpp

```
不同的网段在 OSI 参考模型的物理层进行连接时，数据传输率必须要相同，但是数据链路层的协议可以不相同
```cpp

```
当集线器的一个端口收到数据后，则会将其从所有端口广播出去
```cpp
...
[一条咸鱼晨](https://www.nowcoder.com/profile/107959225)
                                                                    ...
                                                                    hub 的工作原理决定了它在网络中只起到放大和转发的作用，目的是扩大网络的传输范围，不具备信号的定向传输能力，即信号传输的方向是固定的。D 选项的其他所有端口广播出去，不包括传入端口，并且方向一直固定。

发表于 2021-04-09 22:47:58

* * *

## 8

        服务器中经常会见到 tcp 连接有一种 timewait 的状态，一般会持续 2MSL 的时间再释放，在四次挥手中的哪一步的哪一端会变成该状态
...
```
客户端到服务端， 发送包 FIN=M，客户端状态
```cpp

```
服务端到客户端， 发送包 ack=M+1，服务端状态
```cpp

```
服务端到客户端,  发送包 FIN=N，服务端状态
```cpp

```
客户端收到包 FIN=N，客户端状态，并发送 ACK=1，ack=N+1
```cpp
...
[Syndrai](https://www.nowcoder.com/profile/833579668)

                                                                    分 时 日 月 周

发表于 2021-04-20 23:42:34
