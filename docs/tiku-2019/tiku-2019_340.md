...
测试 test,驱动 drive,开发 development
...
Repeatable read 可避免脏读、不可重复读的发生，不是避免可重复读的发生另外 Oracle 默认是 read committed，MySQL 默认是 repeatable read
