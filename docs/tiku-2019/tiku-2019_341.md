...
编辑于 2019-08-15 17:56:14

* * *

[莫笑江湖一别离](https://www.nowcoder.com/profile/64949544)

```cpp
#考的是 python 代码
name = "顺顺" #定义一个变量 name 并赋值“顺顺”,python 变量具有动态类型
#python 是通过制表位的新式分割代码
#定义一个 fun1()函数 开始
def fun1():
    print(name)
#结束
#定义一个空的方法体 fun2()函数 开始
def fun2():
#结束
name = "丰丰" #为变量 name 再次赋值“丰丰”
fun1() #直接调用函数 fun1(), 打印输出  顺顺 ,自动换行
fun1() #直接调用函数 fun1() ,打印输出  顺顺,自动换行  答案没有换行,得出结论编题人是个侮辱 python 和 java 的**
fun2() #直接调用函数 fun2() 
```

发表于 2019-08-27 20:26:37

* * *

[韩少华](https://www.nowcoder.com/profile/894121813)

name="丰丰"是 f2()里的局部变量，而 f1()里的是外部那个 name

发表于 2019-08-14 22:49:51
...
发表于 2019-08-29 14:36:09

* * *

[信自己](https://www.nowcoder.com/profile/5222489)

怎么啥都考？？？java，python  我只会 C++啊

发表于 2019-08-28 20:07:44

* * *

[我该叫什么](https://www.nowcoder.com/profile/8574246)

python 的装饰器内容，decode01,decode02 都是装饰器，同时作用在 func 上。

发表于 2019-03-26 18:56:05

* * *

## 30

下面关于 MapReduce 的 combiner 优化叙述正确的是（   ）
...
参考[文章](https://blog.csdn.net/lb812913059/article/details/79900181)
Hadoop 允许用户针对 map 任务的输出指定一个 combiner（就像 mapper,reducer）。combiner 函数的输出作为 reduce 函数的输入。

发表于 2019-03-13 09:08:06
...
Java 工程师 C++工程师 算法工程师 顺丰科技 Spark

讨论

[之渊](https://www.nowcoder.com/profile/662789664)

首先获取 list 里面的每个元素，不管有没有 map 都算开始第一个阶段，stage1 接着 reducebykey 肯定 shuffle 了。 因此就 有一个 stage2 了而后面的 count 计算 是可以并行处理的，也就是 每个 key 的总数互相之间计算并没有依赖，因此不会产生新的 stage 。其实还可以这样理解 shuffle 肯定会产生 stage . 而 stage 的划分是依赖关系决定的，遇窄依赖不变，遇宽依赖则进行划分。第一个步开始的时候取数生成 RDD 是属于 窄依赖。 也就是 stage 数是 1 接着 reduceByKey  必须按照 key 来进行生成新的分区。与原来开始的时候 rdd 的分区数可能不一样了。 划分一个新的 stage. 也就是 2 ![](img/90c877da4372742fbeb2dd51ad575020.png) 

发表于 2020-08-20 08:32:44

* * *

[佳境 Shmily](https://www.nowcoder.com/profile/899318713)

以 shuffle 为界限划分两个 stage，常见 shuffle 算子 distinct、groupByKey、reduceByKey、aggregateByKey、join、cogroup、repartition 等。

发表于 2020-03-09 14:46:09

* * *

[灵踪马](https://www.nowcoder.com/profile/287133282)
...
一个 spark 作业运行时包括一个 Driver 进程，也是作业的主进程，具有 main 函数，并且有 SparkContext 实例，是程序的入口
功能
    向集群申请资源
    负责作业的调度和解析
    生成 Stage 并调度 Task 到 Executor 上（包括 DAGScheduler，TaskScheduler）

发表于 2020-03-13 10:52:47

* * *

[qnstar](https://www.nowcoder.com/profile/506039296)

Driver**具有 main 函数**，并且有 SparkContext 实例，是程序的入口
...
发表于 2020-10-19 21:38:05

* * *

[莫风尘](https://www.nowcoder.com/profile/733632155)

driver 划分的 task 由 executor 执行

发表于 2019-04-01 18:49:09

* * *

## 33

zookeep 在选举过程中，不会出现的以下哪种选举状态（）
...
Java 工程师 C++工程师 算法工程师 顺丰科技 数据流

讨论

[我该叫什么](https://www.nowcoder.com/profile/8574246)

LOOKING：寻找 Leader 状态，处于该状态需要进入***流程
LEADING：领导者状态，处于该状态的节点说明是角色已经是 Leader
FOLLOWING：跟随者状态，表示 Leader 已经***出来，当前节点角色是 follower
OBSERVER：观察者状态，表明当前节点角色是 observer
没有（wating）等待状态

发表于 2019-03-26 18:59:45

* * *

## 34

Zookeeper 的一致性协议 Zab,不具有以下哪个特点（）
...
Java 工程师 C++工程师 算法工程师 顺丰科技 云计算开发
...
```cpp
调用 Thread 的 sleep()方法会释放锁，调用 wait()方法不释放锁
```

```cpp
一个线程调用 yield 方法，可以使具有相同优先级线程获得处理器
```

```cpp
在 Java 中，高优先级的可运行的线程会抢占低优先级线程的资源
```

```cpp
java 中，线程可以调用 yield 方法使比自己低优先级的线程运行
```

本题知识点

Java 工程师 C++工程师 算法工程师 顺丰科技 Java

讨论

[Bean 冷的心](https://www.nowcoder.com/profile/411517301)

还记得马老师说过，yiled 方法属于高风亮节的行为，这个坑位我不上了，后面跟我同级别的先上厕所。这样比较好记！高优先级的就是你们的县长，县长要上厕所，对不起，你得出来，县长先上，县长上完了，CPU 分配到你了，你才能继续拉 X。

编辑于 2019-07-19 10:50:57
...
发表于 2019-03-26 19:01:28

* * *

[猪头皮·](https://www.nowcoder.com/profile/754636021)

睡觉是抱着锁睡觉的

发表于 2020-03-18 16:13:26
...
发表于 2019-11-11 08:45:55

* * *

[执梗](https://www.nowcoder.com/profile/450534637)

![](img/ab1c263400f5328e367ff8bc88984bcb.png)你让我一个学 Java 的承认 Java 不高效？今天就是前三个不选我都要选第四个！

编辑于 2021-12-10 11:08:55
...
Java 工程师 C++工程师 算法工程师 顺丰科技 Hadoop

讨论

[双十八](https://www.nowcoder.com/profile/906689076)

集群刚启动会启动安全模式，安全模式下，只能读而不能做修改文件的操作。目的是为了保证数据块的完整性，高低于配置文件的副本率的时候会删除或者增加相应副本数。虽然不能进行修改文件的操作，但是可以浏览目录结构、查看文件内容的。以下命令可以控制安全模式的进入、退出、查看。但是建议等集群自动退出安全模式再进行文件操作。命令 hadoop fs -safemode get 查看安全模式状态命令 hadoop fs -safemode enter  进入安全模式状态命令 hadoop fs -safemode leave 离开安全模式

发表于 2020-08-23 16:27:31
...
发表于 2020-07-17 16:54:00

* * *

[牛客 923897924 号](https://www.nowcoder.com/profile/923897924)

安全模式下， 用户可以执行不修改元数据的操作，例如：ls，cat，get 

发表于 2020-03-21 18:00:11
